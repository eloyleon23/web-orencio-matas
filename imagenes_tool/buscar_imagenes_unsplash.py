#!/usr/bin/env python3
"""
Buscar imágenes de productos usando Unsplash API (gratuita)
...
def buscar_imagen_unsplash(access_key, query, sesion):
    """Busca imágenes usando Unsplash API (gratuita)."""
    ...
    try:
        ...
        if resp.status_code == 401:
            return None, "API Key de Unsplash inválida (401)"
        if resp.status_code == 403:
            return None, "cuota de Unsplash agotada (403)"
        if resp.status_code != 200:
            return None, f"Unsplash HTTP {resp.status_code}"
        ...
        return urls.get("regular") or urls.get("small"), "Unsplash API OK"
...
def main():
    ...
    ap.add_argument("--unsplash-key", required=True,
                     help="Access Key de Unsplash (gratuita en unsplash.com/developers)")
    ap.add_argument("--salida", default="imagenes_unsplash",
    ...
    for i, p in enumerate(productos, 1):
        ...
        # Buscar en Unsplash
        url_imagen, metodo = buscar_imagen_unsplash(args.unsplash_key, query, sesion)
