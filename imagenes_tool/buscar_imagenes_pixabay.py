#!/usr/bin/env python3
"""
Buscar imágenes de productos usando Pixabay API (gratuita)
...
def buscar_imagen_pixabay(api_key, query, sesion):
    """Busca imágenes usando Pixabay API (gratuita)."""
    ...
    try:
        ...
        if resp.status_code == 429:
            return None, "cuota horaria de Pixabay agotada (429)"
        if resp.status_code != 200:
            return None, f"Pixabay HTTP {resp.status_code}"
        ...
        return hits[0].get("largeImageURL") or hits[0].get("webformatURL"), "Pixabay API OK"
...
def main():
    ...
    ap.add_argument("--pixabay-key", required=True,
                     help="API Key de Pixabay (gratuita en pixabay.com/api/docs)")
    ap.add_argument("--salida", default="imagenes_pixabay",
    ...
    for i, p in enumerate(productos, 1):
        ...
        # Buscar en Pixabay
        url_imagen, metodo = buscar_imagen_pixabay(args.pixabay_key, query, sesion)
